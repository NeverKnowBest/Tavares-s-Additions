...
public class TavaressAdditionsModBiomes {
	@SubscribeEvent
			...
			if (dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					addParameterPoint(parameters, new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(1f, 2f), Climate.Parameter.span(-1f, -0.4f), Climate.Parameter.span(-1f, 1f), Climate.Parameter.span(-1f, 1f),
							Climate.Parameter.span(0.2f, 0.9f), Climate.Parameter.span(-1f, 1f), 0), biomeRegistry.getHolderOrThrow(ResourceKey.create(Registries.BIOME, new ResourceLocation("tavaress_additions", "bettle_nest")))));
					chunkGenerator.biomeSource = MultiNoiseBiomeSource.createFromList(new Climate.ParameterList<>(parameters));
					chunkGenerator.featuresPerStep = Suppliers
							.memoize(() -> FeatureSorter.buildFeaturesPerStep(List.copyOf(chunkGenerator.biomeSource.possibleBiomes()), biome -> chunkGenerator.generationSettingsGetter.apply(biome).features(), true));
				}
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					((TavaressAdditionsModNoiseGeneratorSettings) (Object) noiseGenerator.settings.value()).settavaress_additionsDimensionTypeReference(dimensionType);
				}
			}
		}
	}
	...
	private static SurfaceRules.RuleSource injectOverworldSurfaceRules(SurfaceRules.RuleSource currentRuleSource) {
		...
		customSurfaceRules.add(anySurfaceRule(ResourceKey.create(Registries.BIOME, new ResourceLocation("tavaress_additions", "bettle_nest")), TavaressAdditionsModBlocks.DENSE_SANDSTONE.get().defaultBlockState(),
				TavaressAdditionsModBlocks.DENSE_SANDSTONE.get().defaultBlockState(), TavaressAdditionsModBlocks.DENSE_SANDSTONE.get().defaultBlockState()));
	...
	public interface TavaressAdditionsModNoiseGeneratorSettings {
		void settavaress_additionsDimensionTypeReference(Holder<DimensionType> dimensionType);
	}
}